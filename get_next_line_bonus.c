...
/*   Updated: 2021/11/15 17:11:58 by agrenon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*make_this_string_happen(int fd, char *str, ssize_t *err)
{
	int		read_numb;
	char	*new_str;
	char	*temp;
...
char	*ft_the_answer(char *temp, char **static_str, ssize_t i, int fd)
{
	char	*next_line;

	next_line = ft_substr(temp, 0, i + 1);
	static_str[fd] = ft_substr(temp, i + 1, ft_strlen(&temp[i]));
	free(temp);
	return (next_line);
}
